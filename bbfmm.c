...
#elif TENSOR	
  FMMCompute(&tree, field, Nf, source, sourceCharge, K, U, VT, Tkz,
	     Ktable, Kweights, Cweights, cutoff, n, dof,
...
#ifdef LINEINT
 void FMMCompute(nodeT **A, vec3 *field, int Nf, segT *segment, vec3 *
...
#ifdef LINEINT            
    /* This is a leaf of the FMM tree.
...
#ifdef TENSOR
    /*
      int Nf = (*A)->Nf, Ns, i, j, k, m;
      int *fieldlist = (*A)->fieldlist, nneigh = (*A)->ineigh,
      *sourcelist;
      ...
      double ihalfL = 2./(*A)->length, tmp1, tmp2, *qsource, fieldval[dof * Nf];
      vec3 fieldt[Nf], fieldpos[Nf], Sf[n*Nf], fcenter = (*A)->center, cshift, 
      *sourcepos;
      nodeT *B;
